...
class Settings(BaseSettings):
    ...
    # اندپوینت‌های /api/advisor/context و /api/advisor/signals هدر «X-Advisor-Key»
    # را الزامی می‌کنند (شکست‌بسته): اگر این کلید تنظیم نشده باشد، این اندپوینت‌ها
    # هیچ داده‌ای برنمی‌گردانند (۴۰۱). چون context با uidِ دلخواهِ بدنه دادهٔ مالیِ
    # هر کاربری را می‌دهد، باید حتماً این کلید در .env سرور و در نودِ HTTP دیفای
    # تنظیم شود تا فقط ورک‌فلو Dify بتواند آن را صدا بزند.
    advisor_api_key: str = ""
    ...
    # شناسهٔ عددیِ تلگرامِ مالک — همیشه ادمینِ پنلِ ربات است و قابل حذف نیست.
    # ادمین‌های بعدی از داخلِ خودِ ربات اضافه/حذف می‌شوند.
    algohub_owner_id: str = "814256165"
    # ادمین‌های اضافیِ ربات (شناسهٔ عددیِ تلگرام، با کاما جدا). این‌ها هم مثلِ
    # مالک به پنلِ ادمینِ ربات و به دکمهٔ «ساختِ تصویرِ نمای کلی بازار» دسترسی
    # دارند. ادمین‌هایی که از داخلِ خودِ ربات اضافه می‌شوند در دیتابیس می‌نشینند؛
    # این فهرست برای ادمین‌های ثابتِ پیکربندی‌شده است.
    algohub_admin_ids: str = "7515255443"

    # ---- ارتباط با بک‌اندِ پنل ژورنال تریدینگ (برای پنلِ ادمینِ ربات) ----
    # ربات برای فعال‌سازیِ اشتراکِ ژورنال و گزارشِ عملکردِ آن، اندپوینت‌های
    # /api/service آن سرویس را صدا می‌زند. توکن باید با SERVICE_TOKEN همان
    # سرویس یکی باشد؛ خالی بودنش یعنی این بخش غیرفعال است.
    journal_api_base: str = "http://127.0.0.1:8000"
